...
'
  'Control' 'waitForRos; roslaunch mrs_uav_general core.launch config_mpc_tracker:=./custom_configs/mpc_tracker.yaml config_constraint_manager:=./custom_configs/constraint_manager.yaml config_uav_names:=./custom_configs/uav_names.yaml config_uav_manager:=./custom_configs/uav_manager.yaml
'
...
'
  'uvdar_estimation' 'waitForControl; sleep 4; roslaunch difec_ron rw_three_sided.launch gui:=false
'
...
'
  'difec_ron' 'waitForControl; roslaunch difec_ron formation_control.launch use_raw_measurements:=false formation_file:='$(rospack find difec_ron)'/tmux_scripts/three_drone_experiment/custom_configs/formation_a.csv
'
